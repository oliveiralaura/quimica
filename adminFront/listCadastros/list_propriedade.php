...
if (!isset($_SESSION['email']) || ($_SESSION['tipo'] !== 'admin' )) {
    ...
    $sql_procede = "SELECT * from propr;";
    $result_procede = $mysqli->query($sql_procede);
    ...
    if (mysqli_num_rows($result_procede) > 0) {
        while ($user = mysqli_fetch_array($result_procede)) {
            $dados_procede[] = array(
                ...
                'propriedade_valor' => $user['propriedade_valor'],
            );
        }
    } else {
        echo 'Nenhum registro de propriedades';
        exit;
    }
?>
<!DOCTYPE html>
...
<header>
        <div class="card">
            ...
            <div class="card-body">
                <a class="nav-link active" aria-current="page" href="../propriedade.php">Cadastrar Propriedade</a>
            </div>
        </div>
    </header>
    ...
    <?php if (isset($_GET['message'])): ?>
        ...
        <div class="row mt-4 rounded m-2 p-2 d-flex border">
            ...
            <div class="col">Nome da Propriedade</div>
            <div class="col">Valor da Propriedade</div>
            <div class="col"></div>
            <div class="col"></div>
        </div>
        <?php foreach($dados_procede as $retorno): ?>
            <div class="row mt-4 rounded m-2 p-2 d-flex border">
                ...
                <div class="col"><?php echo $retorno['propriedade_nome']; ?></div>
                <div class="col"><?php echo $retorno['propriedade_valor']; ?></div>
                <div class="col">
                        <a href="../edit/edit_propriedades.php?id=<?php echo $retorno['id']; ?>" class="btn btn-success">Update</a>
                    </div>

                    <div class="col">
                        <form action="../../adminBack/delete/delete_propriedade.php" method="post" onsubmit="return confirm('Tem certeza que deseja excluir esta propriedade?');">
